...
class NetworkBehavior(nn.Module):
    ...
    def _get_optimal_device(self):
        ...
        is_google_cloud_run = os.getenv('K_SERVICE') is not None
        is_vercel = os.getenv('VERCEL') is not None
        is_local = not is_google_cloud_run and not is_vercel
    ...
    def init_settings(self, is_training):
        ...
        if(is_training):
            ...
            self.eval()
        if(self.use_data_parallel and self.device.type == 'cuda'):
            # Only use DataParallel with CUDA
...
class NetworkInterface:
    ...
    def __init__(self, net, save_name, load_checkpoint=False, load_path='cache_data'):
        ...
        if(os.path.exists(save_path)):
            # ENHANCED GPU ACCELERATION: Load model to optimal device
            ...
            # ENHANCED GPU ACCELERATION: Load checkpoint to optimal device
